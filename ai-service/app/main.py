...
try:
  ...
  raise RuntimeError(f"PIL import failed: {e}")
...
_clip: Optional[Tuple[Any, Any, str, str]] = None  # (model, processor, model_id, device)
...
def _load_clip() -> Tuple[Any, Any, str, str]:
  # Lazy import: avoid importing torch/transformers unless needed.
  try:
    import torch  # type: ignore
    from transformers import CLIPModel, CLIPProcessor  # type: ignore
  except Exception as e:  # pragma: no cover
    raise HTTPException(status_code=500, detail=f"CLIP dependencies not available: {e}")

  model_id = os.environ.get("TIDY_CLIP_MODEL_ID", "openai/clip-vit-base-patch32")
  ...
  try:
    ...
    raise HTTPException(
      status_code=500,
      detail=(
        f"failed to load CLIP model '{model_id}': {e}. "
...
def _get_clip() -> Tuple[Any, Any, str, str]:
  global _clip
...
def clip_text_embed(payload: Dict[str, Any]) -> Dict[str, Any]:
  ...
  model, processor, model_id, device = _get_clip()

  try:
    import torch  # type: ignore
  except Exception as e:  # pragma: no cover
    raise HTTPException(status_code=500, detail=f"torch not available: {e}")

  inputs = processor(text=items, images=None, return_tensors="pt", padding=True, truncation=True)
