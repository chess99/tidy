...
def _import_clip_get_encoder():
    try:
        ...
        try:
            from clip_encoder import get_encoder  # type: ignore
        finally:
            if added_app_dir:
                try:
...
def _get_clip():
    ...
    with _clip_load_lock:
        if _clip is not None:
            return _clip
        try:
            get_encoder = _import_clip_get_encoder()
        except Exception as e:  # pragma: no cover
...
def _missing_python_dependency(module_name: str) -> Optional[str]:
    try:
    ...
    try:
        ...
        return exc.name or module_name
    except Exception as exc:
        return f"{module_name}: {exc}"
    return None


def _missing_clip_runtime_dependency() -> Optional[str]:
    for module_name in ("torch", "transformers"):
        missing_dependency = _missing_python_dependency(module_name)
...
def _clip_capability() -> Dict[str, Any]:
    missing_dependency = _missing_clip_runtime_dependency()
    if missing_dependency is not None:
        return {
            "available": False,
            "code": "clip_encoder_unavailable",
            "message": f"CLIP encoder dependency unavailable: {missing_dependency}",
        }
    try:
        _import_clip_get_encoder()
    except Exception as e:
        return {
            "available": False,
            "code": "clip_encoder_unavailable",
