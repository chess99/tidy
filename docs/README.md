...
- [`docs/智能搜索_CLIP.md`](智能搜索_CLIP.md)：**智能搜索（CLIP）**（文本搜图 + 相似图片，阈值语义与索引/任务流；含 `/api/files` smartQuery profiling 用法 + 交互防连发与 ai-service 并发控制）。
- [`docs/性能排查指南.md`](性能排查指南.md)：**性能排查指南**（端到端定位套路：耗时/CPU/内存/排队；含可复现脚本入口）。
- [`docs/系统管理UI.md`](系统管理UI.md)：**系统管理界面**（配置与任务队列一屏完成，immich 风格）。
- **详情找相似**：在详情面板点“找相似(pHash)”或“相似(CLIP)”并同步到左侧筛选（结果展示在“全部文件”）。
...
- `site/`：**项目主页**（GitHub Pages 落地页，含 `versions.html` 版本对照）。
